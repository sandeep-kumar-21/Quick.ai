  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if(data.success){
        ...
        console.log("generate image else blok error")
        toast.error(data.message)
      }
    } catch (error) {
      console.log("Generate image catch blok : " + error.message)
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <textarea onChange={(e) => setInput(e.target.value)} value={input} rows={4} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder='Descirbe what you want to see in the image...' required/>
